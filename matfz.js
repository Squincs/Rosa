// Berk -Y-#4188 tarafından Matfz için kodlanmıştır.

const Discord = require("discord.js")
const client = new Discord.Client()
const ayarlar = require("./ayarlar.json")
const moment = require("moment")//Berk -Y-#4188
const fs = require("fs")
...
fs.readdir('./commands/', (err, files) => { 
  ...
  console.log(`${files.length} komut yüklenecek.`)//Berk -Y-#4188
  files.forEach(f => {                    
...
client.on('message', async message => {
  ...
  if(message.content === '.tag') {
    message.channel.send(`\`${ayarlar.tag}\``)//Berk -Y-#4188
  }
  })
...
client.on("guildMemberAdd", async (member) => {
  let arwSayılar = {
    ...
    "9": `${ayarlar.dokuzEmoji}`//Berk -Y-#4188
}
...
const arw = `Ve senin hesabın sunucumuza kayıt olmak için tüm şartları karşılıyor! ${client.emojis.cache.get(ayarlar.yes)}`//Berk -Y-#4188
if(arwZaman < 1296000000) {
  arw = `Ve senin hesabın sunucumuza kayıt olmak için daha çok genç! ${client.emojis.cache.get(ayarlar.no)}`//MBerk -Y-#4188
}
  
member.roles.add(ayarlar.kayıtsızRol)
member.roles.add(ayarlar.kayıtsızRol)
member.roles.add(ayarlar.kayıtsızRol)//Berk -Y-#4188
